...
Это осознанно **не** попытка довести проект до максимально строгой Clean Architecture, Hexagonal Architecture или FSD-подобной декомпозиции любой ценой.

Для этого проекта важнее не количество формальных границ и абстракций само по себе, а их практическая окупаемость:

- если новый слой, порт, адаптер или обёртка действительно упрощает сопровождение, тестирование, замену инфраструктуры или снижает риск регрессий, он оправдан;
- если абстракция добавляет церемонию, размазывает логику по большому количеству файлов и повышает когнитивную нагрузку без соразмерной пользы, мы её не вводим;
- допустимы прагматичные компромиссы, пока они не разрушают читаемость, трассируемость потока данных и разделение ответственности на уровне здравого смысла;
- архитектурные границы в проекте важны, но они не догма: предпочтение отдаётся явному, понятному и сопровождаемому коду, а не абстракциям ради абстракций.

Иными словами, проект придерживается **Pragmatic DDD**: мы сохраняем дисциплину слоёв и моделей там, где она помогает, но не стремимся к "архитектурной чистоте" ценой одиночной сопровождаемости.
...
* **База данных:** PostgreSQL 18 (Dev/Test/Prod).
...
* **База данных (PostgreSQL 18):**
  * **Единый backend:** PostgreSQL является единственной поддерживаемой runtime и test БД. Приложение и Alembic принимают только `postgresql+asyncpg` URL; SQLite compatibility layer не поддерживается.
  * **Идентификаторы:** Внутренние PK/FK используют `BigInteger`. Бизнес-значения вроде баллов, сумм и порогов остаются `Integer`.
  * **Enums:** Схемные enum-поля используют именованные PostgreSQL native `ENUM`. Общие типы переиспользуются между таблицами и хранят имена членов Python enum.
  * **Миграции:** Активная история Alembic начинается с PostgreSQL initial baseline и не связана со старой SQLite-цепочкой. Baseline применяется к новой PostgreSQL БД; автоматический перенос SQLite-данных не предоставляется.
  * **Runtime:** Локальный и production Compose используют отдельный `postgres:18-alpine` service, persistent data volume и отдельный volume для custom-format backup.
  * **Тестирование:** Integration-тесты выполняются на PostgreSQL 18 через Testcontainers или безопасный `PYBOT_TEST_DATABASE_URL`; схема создаётся реальным `alembic upgrade head`.
